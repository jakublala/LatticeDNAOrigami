...
"""A script to create input json files from sequences for double-removal (DR) simulations"""

SCAFFOLD_SEQFILE = 'inps/snodin_scaffold.seq'
...
def generate_JSONfile(removedStaples):
    STAPLE_SEQFILE = 'seqs/snodin_staples_{}_{}.seq'.format(removedStaples[0], removedStaples[1])
    ...
    OUTPUT_FILENAME = 'jsons/snodin_unbound_{}_{}.json'.format(removedStaples[0], removedStaples[1])
...
CYCLIC = False # set depending on the DNA origami design
...
# Iterate through all simulation setups
for staples in simIndices:
    generate_JSONfile(staples)

# No staple removed simulation setup
generate_JSONfile([0, 0])
